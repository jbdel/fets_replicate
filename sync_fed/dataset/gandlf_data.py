...
def get_appropriate_file_paths_from_subject_dir(dir_path,
                                                ...
                                                allowed_labelfile_endings=["_seg_binary.nii.gz",
                                                                           "_seg_binarized.nii.gz",
                                                                           "_SegBinarized.nii.gz", "_seg.nii.gz"],
        ...
        if include_labels:
            ...
            if allowed_label and not excluded_label:
                if return_dict[label_tag] is None:
                    ...
                    raise RuntimeError(
                        'Found two label files (allowing any of {} and excluding any of {}) in directory {} '.format(
                            allowed_labelfile_endings, excluded_labelfile_endings, dir_path))
...
class GANDLFData(object):
    ...
    def setup_for_train_val(self):
        ...
        if (newly_lost_train != [] or newly_lost_val != []) and self.force_rerun_with_recent_data_loss:
            raise ValueError(
                'Train with UIDs: {}, and  Val with UIDs: {} are newly missing, and force_rerun_with_recent_data_loss is True. Put this data back into {} and re-run, or re-run without them.'.format(
                    newly_lost_train, newly_lost_val, self.data_path))
    ...
    def get_subdirs_from_uids(self, uids):
        if self.uids_to_subdirs is None:
            raise ValueError(
                'Asking for self.uids_to_subdirs before calling self.get_data_uids_from_disk where this attribute is set.')
        else:
    ...
    def sanity_check_split_info(self):
        ...
        if os.path.exists(self.split_instance_dirpath):
            ...
            if not np.all(exists_in_all):
                raise ValueError(
                    'At least one of {} is missing!! Carefully recover, using information produced in model_and_data.log during last run.'.format(
                        need_all))
            if np.any(exists_in_all_or_none) and not np.all(exists_in_all_or_none):
                raise ValueError(
                    'All or none of: {} should exists, but exactly one was found!! Carefully recover, using information produced in model_and_data.log during last run.'.format(
                        need_all_or_none))
            ...
            if lost_train != []:
                self.logger.debug(
                    '\nWARNING: Training data with UIDs: {} from split: {} now missing on disk.\n'.format(lost_train,
                                                                                                          self.split_instance_dirname))
                if not self.handle_data_loss_from_previous_split:
                    raise ValueError(
                        'Training data with UIDs: {} from split: {} now missing on disk and handle_data_loss_from_previous_split is False.'.format(
                            lost_train, self.split_instance_dirname))
            if lost_val != []:
                self.logger.debug(
                ...
                if not self.handle_data_loss_from_previous_split:
                    raise ValueError(
                        'Validation data with UIDs: {} from split: {} now missing on disk and handle_data_loss_from_previous_split is False.'.format(
                            lost_val, self.split_instance_dirname))
    ...
    def record_split_info(self, train, val):
        ...
        if self.num_train_historical_assignments + self.num_val_historical_assignments != 0:
            self.logger.debug(
                '{} of these were known to be used previously, so were assigned to train/val using historical split info.'.format(
                    self.num_train_historical_assignments + self.num_val_historical_assignments))
        if self.allow_previously_unseen_data and self.num_fresh_to_split != 0:
            self.logger.debug(
                '{} of these were previosly unknown, so assigned randomly (with best effort to maintain percent train:{}.'.format(
                    self.num_fresh_to_split, self.percent_train))
        elif not self.allow_previously_unseen_data and (
                self.num_on_disk > self.num_train_historical_assignments + self.num_val_historical_assignments):
            self.logger.debug(
                'Not allowing previously unseen data samples into split, since old split info exists and allow_new_data_into_previous_split is False.')
    ...
    def record_lost_data(self, lost_train, lost_val):
        ...
        if (not lists_empty) or old_lost_info_present:
            self.logger.debug('\nLost data info:\n')
            self.logger.debug(
                'Data with UIDs: {} were previously used for training but now missing from {}'.format(lost_train,
                                                                                                      self.data_path))
            self.logger.debug(
                'Data with UIDs: {} were previously used for validation but now missing from {}\n'.format(lost_val,
                                                                                                          self.data_path))
    ...
    def check_for_undesirable_split(self, train, val):
        ...
        if (len(train) == 0) or (len(val) == 0):
            raise ValueError(
                'Split (accounting for percent_train, historical train/val assignments, and complete data subdirectories under data_path) results in an empty train or val set.')

    def set_train_and_val_loaders(self, train_dataframe, val_dataframe):
        self.train_loader, self.penalty_loader = self.get_loaders(data_frame=train_dataframe, train=True,
    ...
    def set_dataframe_headers(self, headers, list_needed=False):
        ...
        if list_needed:
            self.headers_list = [self.headers['subjectIDHeader']] + self.headers['channelHeaders'] + [
                self.headers['labelHeader']] + self.headers['predictionHeaders']
    ...
    def get_sorted_subdirs(self):
        ...
        subdirs_list = np.sort([item for item in list_dir if item not in self.excluded_subdirs and os.path.isdir(
            os.path.join(self.data_path, item))])

        self.logger.debug(
            "\nFound {} subdirectories under {} excluding the subdirectories that were supposed to be ignored: {}\n".format(
                len(subdirs_list), self.data_path, self.excluded_subdirs))
    ...
    def infer_with_patches(self, model_inference_function, features):
        ...
        for patches_batch in patch_loader:
            ...
            pred_mask = model_inference_function[0](model_inference_function=model_inference_function[1:],
                                                    features=image)
    ...
    def infer_with_crop_and_patches(self, model_inference_function, features):
        ...
        return self.infer_with_crop(model_inference_function=[self.infer_with_patches] + model_inference_function,
                                    features=features)
    ...
    def infer_with_crop(self, model_inference_function, features):
        ...
        small_idx_corner, large_idx_corner, cropped_features_np = crop_image_outside_zeros(array=squeezed_features_np,
                                                                                           psize=self.psize)
        ...
        # perform inference
        output_of_cropped = model_inference_function[0](features=final_features,
        ...
        if (len(prelim_shape) != 5) or (prelim_shape[0] != 1) or (prelim_shape[1] != len(self.class_list)):
            raise ValueError(
                'Expected shape [1, num_classes, spacial_x, spacial_y, spacial_z] of cropped-feature output and found ',
                output_of_cropped.shape)
        ...
        output[:, :, small_idx_corner[0]:large_idx_corner[0], small_idx_corner[1]:large_idx_corner[1],
        small_idx_corner[2]:large_idx_corner[2]] = \
            output_of_cropped[:, :, :large_idx_corner[0] - small_idx_corner[0],
            :large_idx_corner[1] - small_idx_corner[1], :large_idx_corner[2] - small_idx_corner[2]]
    ...
    def write_outputs(self, outputs, dirpath, class_list, class_axis=1):
        for idx, output in enumerate(outputs):
            ...
            if output.shape[class_axis] != len(class_list):
                raise ValueError(
                    'The provided output does not enumerate classes along axis {} as assumed.'.format(class_axis))
            ...
            if self.class_list == [0, 1, 2, 4]:
                ...
                if not np.all(np.sum(output, axis=class_axis) - 1 < 1e-6):
                    raise ValueError(
                        'The provided output does not appear to have softmax along {} axis as assumed.'.format(
                            class_axis))
            ...
            if list(new_output.shape) != [155, 240, 240]:
                raise ValueError(
                    'Unexpected shape {} during processing of output image for sitk savings (was expecting [155, 240, 240]).'.format(
                        new_output.shape))
